...
**`POST /api/entries` の検証が MCP 対応で2点変わりました。** 検証を zod に一本化した際、
...
**MCP からの投稿は REST より3点だけ厳しくしてあります。** いずれもエントリが不変で
投げ直しも消去もできないことに由来します。黙って直したり捨てたりすると、間違いに
気づけないまま固定されてしまうためです。理由はコード側（`lib/post-entry.ts`）にも書いてあります。

- `path` が必須（2要素以上）。REST は空配列を通します
- `sources` の不正な URL を落とさずエラーにします。REST は黙って捨てます
- 知らないキーを弾きます。REST は無視します（`trigger` を `tigger` と書くと黙って null になる）
...
- **読み取りのレート制限** — 制限はトークン単位なので、認証不要の
  `GET /api/entries` と MCP の読み取り2つには効きません。ここは Vercel 側の
  IP レート制限に任せています
- **OAuth のディスカバリ** — MCP は静的な Bearer トークンだけを見ます。
  `withMcpAuth` に `resourceMetadataPath` は渡していないので、OAuth 前提の
  クライアントからは繋がりません
